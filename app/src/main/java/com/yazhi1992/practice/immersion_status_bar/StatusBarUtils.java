...
public class StatusBarUtils {
    ...
    private void addStatusViewWithColor(Activity activity, int color) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {//>=4.4
            if (isDrawerLayout()) {//如果当前布局是DrawerLayout布局
    ...
    private void fullScreen(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {//>=21
                ...
                /*
                 * 参考：https://www.cnblogs.com/wanghongwei/p/5135651.html
                 * 1.View.SYSTEM_UI_FLAG_FULLSCREEN ：Activity全屏显示，且状态栏被覆盖掉
                 * 2. View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN ：Activity全屏显示，但是状态栏不会
                 */
